...
import { FaSearch, FaBook, FaHistory, FaTwitter, FaYoutube, FaPodcast, FaImage, FaTrash, FaComment, FaAngleLeft, FaAngleRight } from "react-icons/fa";
...
export default function Home() {
  ...
  const [isHistoryOpen, setIsHistoryOpen] = useState(false); // Toggle for both mobile and desktop
  ...
  const checkFacts = debounce(
    async (queryInput?: string): Promise<FactCheckResult | null> => {
      ...
      try {
        if (navigator.onLine) {
          ...
          const offlineResult: FactCheckResult = { claim: `Offline Mode: ${offlineRules.rules}\nInput: "${query}" appears unchecked.` };
          setResult(offlineResult);
          return offlineResult;
        } else {
          const offlineResult: FactCheckResult = { claim: "Offline mode unavailable during prerendering." };
          setResult(offlineResult);
          return offlineResult;
        }
      } catch (error) {
        const errorResult: FactCheckResult = { claim: "Error checking facts. Please try again." };
  ...
  const handleFactCheck = async () => {
    const factCheckResult = await checkFacts();
    if (factCheckResult) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col p-4 font-sans">
      ...
      <div className="md:hidden bg-gradient-to-br from-purple-600 to-violet-600 text-white p-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <button onClick={() => setIsHistoryOpen(!isHistoryOpen)} className="text-2xl">
          <FaHistory />
        </button>
      </div>
      ...
      <div className="flex flex-1 md:flex-row">
        ...
        <motion.div
          animate={{ width: isHistoryOpen ? "20rem" : "4rem" }}
        ...
        >
          ...
          >
            {isHistoryOpen ? <FaAngleLeft /> : <FaAngleRight />}
          </button>
          <div className="flex items-center justify-center mb-4">
            <FaHistory className="text-2xl" />
            {isHistoryOpen && <span className="ml-2 text-xl font-semibold">History</span>}
          </div>
          <div className={isHistoryOpen ? "flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-400 scrollbar-track-transparent" : "hidden"}>
