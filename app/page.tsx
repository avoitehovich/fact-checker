...
export default function Home() {
  ...
  const [history, setHistory] = useState<string[]>([]); // Initialize empty, load later
  ...
  // Load history and offline cache on client mount
  useEffect(() => {
    ...
    async (queryInput?: string): Promise<FactCheckResult | null> => {
      ...
      try {
        if (navigator.onLine) {
          ...
          setResult(resultObj);
          return resultObj;
        } else if (typeof window !== "undefined") { // Check for client-side
  ...
  const handleHistoryClick = async (item: string) => {
    ...
    if (result !== undefined) {  // Handle both null and undefined
